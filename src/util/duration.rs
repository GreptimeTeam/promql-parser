...
lazy_static! {
    ...
    // match `ms` first, to avoid conflict with `m`
        r"(?x)
(?P<unit>
    (?P<ms>[0-9.]+)ms |
    (?P<y>[0-9.]+)y |
...
///
/// MetricsQL Enhancement:
/// 1. Duration may be fractional: `1.5s`
/// 2. The duration suffix is optional. The duration is in seconds if the suffix is missing: `1.5 == 1500ms`
/// 3. Use `i` to indicate `step`, `step` must be `Some` if `i` in `ds`: `1.5i == 1.5 * step`
///
/// # Examples
///
/// Basic usage:
...
///
/// assert_eq!(util::parse_duration("1h", None).unwrap(), Duration::from_secs(3600));
/// assert_eq!(util::parse_duration("4d", None).unwrap(), Duration::from_secs(3600 * 24 * 4));
/// assert_eq!(util::parse_duration("4d1h", None).unwrap(), Duration::from_secs(3600 * 97));
/// ```
pub fn parse_duration(ds: &str, step: Option<Duration>) -> Result<Duration, String> {
    if ds.is_empty() {
    ...
    if ds.contains('i') && step.is_none() {
        return Err("use `i` in duration string but not step given".into());
    }
    ...
    if let Ok(v) = ds.parse::<f64>() {
        return Ok(Duration::from_millis((v * 1000_f64) as u64));
    }
    ...
    let dur = DURATION_RE
        ...
        .flat_map(|caps| {
            let interval = caps
                ...
                .and_then(|v| {
                    let step = step?;
                    Some(Duration::from_millis((step.as_millis() as f64 * v) as u64))
                });
            ALL_CAPS
                .into_iter()
                .map(move |(title, duration)| {
                    caps.name(title)
                        .and_then(|cap| cap.as_str().parse::<f64>().ok())
                        .map(|v| Duration::from_millis((duration.as_millis() as f64 * v) as u64))
                })
...
mod tests {
    ...
    fn test_valid_duration() {
        ...
        for (s, expect) in ds {
            let d = parse_duration(s, Some(DAY_DURATION));
            assert!(d.is_ok());
    ...
    fn test_invalid_duration() {
        let ds = vec!["-1w", "d", "", "0", "0w", "0s"];
        for d in ds {
            assert!(
                parse_duration(d, None).is_err(),
                "{} is invalid duration!",
                d
            );
        }
