  ...
  0.5"#,
    ...
    fn test_prettify_with_utf8_labels() {
        ...
        let cases = vec![
            ...
            (r#"foo{"label.with.dots"="value"}"#, r#"foo{"label.with.dots"="value"}"#),
            (r#"bar{"label-with-dashes"="test"}"#, r#"bar{"label-with-dashes"="test"}"#),
            (r#"baz{"label:with:colons"="data"}"#, r#"baz{"label:with:colons"="data"}"#),
            (r#"sum by ("service.version", foo) ({"some.metric"})"#, r#"sum by ("service.version", foo) ({__name__="some.metric"})"#),
            (r#"sum by (`service.version`, foo) ({"some.metric"})"#, r#"sum by ("service.version", foo) ({__name__="some.metric"})"#),
            // Regular labels should not be quoted
            (r#"foo{job="web"}"#, r#"foo{job="web"}"#),
            (r#"bar{instance_id="server1"}"#, r#"bar{instance_id="server1"}"#),
        ];
